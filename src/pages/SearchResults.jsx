...
function SearchResults() {
  ...
  const { data, isLoading,isFetching, error } = useGetReciterQuery({id: qurey, lang});
  const { data: sur } = useGetAllSurahDetailsQuery(lang);
  ...
  useEffect(() => {
    const generateSurahData = async () => {
      if (!moshaf?.surah_list || !moshaf?.server || !sur) return;

      const surahPromises = moshaf.surah_list.split(",").map(async (num) => {
        const surahDetail = sur?.suwar.find((surah) => surah.id == num);
    ...
    <div className="flex flex-col bg-second-black rounded-lg  w-full min-h-screen ">
      {isLoading || isFetching ? (
        ...
        <>
          ...
          <div className="down">
            <div className="actions flex items-center gap-6 p-5">
